...
1. **Read-only against GitHub.** REST reads use GET. Closing-issue relationships and bulk corpus reads use fixed GraphQL queries (HTTP POST, never mutations); variables cannot change their operations. Bulk PR heads use Git fetch. Nothing here labels, comments on, closes or merges. Anything that would is a separate, explicitly authorized piece of work that defaults to `--dry-run`; see `README.md`'s "Current limits".
2. **Scripts own managed data.** The TUI never writes ledgers, groups, duplicate verdicts, batches, cache evidence, corpora, watches, tracking records or notification state directly; it invokes their owning scripts through `runScript` or cancellable `runReadScript`. Its direct writes are limited to TUI configuration and a disposable context export when clipboard copy fails. No new code path may bypass the owning script.
