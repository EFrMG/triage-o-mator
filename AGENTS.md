...
1. **GitHub reads and explicitly approved comments and state changes.** REST reads use GET. Closing-issue relationships and bulk corpus reads use fixed GraphQL queries (HTTP POST, never mutations); variables cannot change their operations. Bulk PR heads use Git fetch. Only `bin/comment` may publish conversation comments and close or reopen an item after an explanatory comment, after explicit approval of the exact text, target and state change; it defaults to dry-run and records comment and state outcomes separately from ledger approval. Nothing here labels or merges. Further write operations are separate, explicitly authorized work that defaults to `--dry-run`; see [Not Built (yet)](README.md#not-built-yet).
