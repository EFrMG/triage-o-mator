...
4. **Archived:** once a maintainer has decided, archive the group; it stays at the end of the list for reference.
...
7. On an item, `m` opens the Duplicates screen; its `b` creates a group from the item and the candidates you ticked, with their title similarity in the notes (see the [README](../README.md#duplicates)).
...
All mutations require an explicit contributor identity. Commands return JSON except Markdown exports. Copy the stable `id` from `create` or `list` for subsequent commands.
