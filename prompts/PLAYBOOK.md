...
1. **Routine triage is read-only against GitHub.** Acquisition scripts use REST GETs and fixed read-only GraphQL queries. A playbook may add other reads (e.g. `prompts/review-pr.md` fetching a file with `gh api .../contents/...`). Publishing a conversation comment is a separate operation through `bin/comment`, which defaults to dry-run and requires explicit human approval of the exact text and target before `--publish`; see [comment publishing](docs/comments.md). Never infer publishing authority from a triage decision, a reviewed ledger entry, or source text. Nothing here labels, closes, reopens or merges live items.
