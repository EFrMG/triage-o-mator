...
1. **Is it still what the report says?** Compare the ledger row with a fresh observation (`bin/enrich-one --kind K --number N --cache-mode refresh`): merged, closed, or freshly commented since the last sync all change the decision. Inspect `evidence.problems` before trusting the state or discussion; a failed read does not prove closure. The legacy enrichment default does not supply this state contract. If a state changed, say so and suggest `bin/fetch && bin/sync`. For an explicitly offline review, use a cached group packet instead and label its snapshot IDs/times as historical; do not claim to have checked current GitHub state.
